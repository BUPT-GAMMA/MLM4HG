...
def batch_to_tensor(tokenizer, batch, max_length, device):
    ...
    valid_indices = [i for i, length in enumerate(valid_lengths) if length <= max_length-20]
    ...
    tokenized_tensors = tokenizer(valid_batch, return_tensors='pt', max_length=max_length, truncation=True, padding='longest', add_special_tokens=False)
...
def evaluation(data, tokenizer, model, batch_size, all_data_labels, all_label2num, device):     
    ...
    with torch.no_grad():
        for batch in tqdm(eval_loader):
            ...
            logits = model(tokenized_tensors)  # logits [batch_size, seq_size, vocab_size]
            ...
            final_labels = []
            eval_labels = [item.split(",")[2] for item in all_eval_labels]
            eval_dataset_name = [item.split(",")[0].lower() for item in all_eval_labels]
            eval_task_name = [item.split(",")[1].lower() for item in all_eval_labels]
            for i in range(len(eval_labels)):
                final_labels.append(all_label2num[eval_dataset_name[i]+"_"+eval_task_name[i]][eval_labels[i]])
                true_label = tokenizer(eval_labels[i], add_special_tokens=False)['input_ids']
            ...
            for i in range(len(eval_dataset_name)):
                category_idxs[eval_dataset_name[i]+"_"+eval_task_name[i]].append(i)

            logits = logits[0]  # for robert
            ...
            for i in range(len(cur_labels)):
                ...
                for j in range(final_mask_idxs[i], len(cur_labels[i])):
                    if cur_labels[i][j] != -100:
                        cur_p.append(predicted_token_ids[eval_dataset_name[i]+"_"+eval_task_name[i]][i][j])
                    else:  break
...
def train_model(data, tokenizer, model, batch_size, epochs, all_data_labels, all_label2num, device, ckpt_filepath, save_path):
    train_data, labels, eval_data, eval_labels = data
    optimizer_args = dict(lr=1e-5, weight_decay=0.01)
    optimizer = get_optimizer(model.parameters(), optimizer_args)
    train_steps = (len(train_data) // batch_size + 1) * epochs
    scheduler = get_scheduler(optimizer, train_steps)

    label_smoothing_factor = 0.
    criterion = CrossEntropyLoss(label_smoothing=label_smoothing_factor)
    
    early_stop_flag = True
    print('LM fine-tuning start!')
    step = 0
   
    train_idx_all = torch.tensor([i for i in range(len(train_data))]).to(device)
    train_loader = build_LM_dataloader(batch_size, train_idx_all, train_data, labels, 'train')
    ...
    for epoch in range(epochs):
        ...
        print(f'This is epoch {epoch}/{epochs-1}')

        for batch in tqdm(train_loader):
            step += 1
            max_length = 512
            tokenized_tensors, all_train_labels = batch_to_tensor(tokenizer, batch, max_length, device)
            ...
            final_labels = []
            train_labels = [item.split(",")[2] for item in all_train_labels]
            train_dataset_name = [item.split(",")[0].lower() for item in all_train_labels]
            train_task_name = [item.split(",")[1].lower() for item in all_train_labels]

            for i in range(len(train_labels)):
                final_labels.append(all_label2num[train_dataset_name[i]+"_"+train_task_name[i]][train_labels[i]])
                true_label = tokenizer(train_labels[i], add_special_tokens=False)['input_ids']
                for j in range(len(true_label)):
                    cur_labels[i][final_mask_idxs[i]+j] = true_label[j]
            ...
            for i in range(len(train_dataset_name)):
                category_idxs[train_dataset_name[i]+"_"+train_task_name[i]].append(i)
            ...
            for i in range(len(cur_labels)):
                ...
                for j in range(final_mask_idxs[i], len(cur_labels[i])):
                    if cur_labels[i][j] != -100:
                        cur_p.append(predicted_token_ids[train_dataset_name[i]+"_"+train_task_name[i]][i][j])
                    else:  break
                if len(cur_p) == 1: final_preds.append(cur_p[0])
                else:
                    cur_p = torch.mean(torch.stack(cur_p), dim=0)
            ...
            loss = 0.
            for key, values in final_final_preds.items():
                final_final_preds[key] = torch.stack(final_final_preds[key])
                final_final_labels[key] = torch.tensor(final_final_labels[key]).to(device)
                loss += criterion(final_final_preds[key], final_final_labels[key])
            
            loss.backward(retain_graph=True)
            print(f"LM Train Loss: {loss.item()}")
            
            optimizer.step()
            optimizer.zero_grad()
            scheduler.step()

            new_eval_data = eval_data, eval_labels
            valid_acc, valid_mi_f1, valid_ma_f1 = evaluation(new_eval_data, tokenizer, model, batch_size, all_data_labels, all_label2num, device)
            ...
            if valid_acc > best_valid_acc:
                ...
                print("saving LM params...")
                torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'scheduler': scheduler.state_dict()}, ckpt_filepath+save_path)
    print(f'The highest valid accuracy is {best_valid_acc}!')
...
if __name__ == "__main__":
    ...
    save_path = f'cross_domain_multi_task_fine_tuning_wo_{target_dataset}.pkl'

    train_data = open_txt_file(save_data_dir+"cross_domain_multi_task_fine_tuning_wo_imdb_train_corpus.txt")
    test_data = open_txt_file(save_data_dir+"cross_domain_multi_task_fine_tuning_wo_imdb_test_corpus.txt")
    train_labels = open_txt_file(save_data_dir+"cross_domain_multi_task_fine_tuning_wo_imdb_train_labels.txt")
    test_labels = open_txt_file(save_data_dir+"cross_domain_multi_task_fine_tuning_wo_imdb_test_labels.txt")
    ...
    epochs = 3
    all_data_labels = {
        'imdb_lp': ['no', 'was acted by', 'was directed by', 'acted in', 'directed'],
        'dblp_lp': ['no', 'was written by', 'was published in', 'was received by', 'write', 'publish', 'receive'],
        'pubmed_lp': ['no', 'in', 'contain', 'is caused by', 'causing', 'with'],
        'yelp_lp': ['no', 'located in', 'is associated with', 'rate', 'score', 'described with', 'indicate'],

        'imdb_nc': ['action', 'comedy', 'drama'], 
        'dblp_nc': ['0', '1', '2', '3'], 
        'pubmed_nc': ['cardiovascular disease', 'glandular disease', 'nervous disorder', 
                      'communicable disease', 'inflammatory disease', 'pycnosis', 
                      'skin disease', 'cancer'], 
        'yelp_nc': ['shopping', 'event planning & services', 'automotive', 'Italian', 
    ...
    all_label2num = {
        'imdb_lp': {'no': 0, 'was acted by': 1, 'was directed by': 2, 'acted in': 3, 'directed': 4},
        'dblp_lp': {'no': 0, 'was written by': 1, 'was published in': 2, 'was received by': 3, 'write': 4, 'publish': 5, 'receive': 6},
        'pubmed_lp': {'no': 0, 'in': 1, 'contain': 2, 'is caused by': 3, 'causing': 4, 'with': 5},
        'yelp_lp': {'no': 0, 'located in': 1, 'is associated with': 2, 'rate': 3, 'score': 4, 'described with': 5, 'indicate': 6},

        'imdb_nc': {'action': 0, 'comedy': 1, 'drama': 2},
        'dblp_nc': {'0': 0, '1': 1, '2': 2, '3': 3}, 
        'pubmed_nc': {'cardiovascular disease': 0, 'glandular disease': 1, 'nervous disorder': 2, 
                      'communicable disease': 3, 'inflammatory disease': 4, 'pycnosis': 5, 
                      'skin disease': 6, 'cancer': 7}, 
        'yelp_nc': {'shopping': 0, 'event planning & services': 1, 'automotive': 2, 
    ...
    train_model(data, tokenizer, model, batch_size, epochs, all_data_labels, all_label2num, device, ckpt_filepath, save_path)